...
const elementVariants = cva(`grid relative place-items-center h-fit w-fit shadow-secondary`, {
  variants: {
    variant: {
...
const ElementType = ({ variant, rounded, size }: ElementTypeProps) => {
  return (
    <div className={cn(elementVariants({ variant, rounded, size }))}>
      <span className="z-20">{variant}</span>
      <GradientBackground intensity="medium" variant="dark" className="z-10" />
    </div>
  );
};
